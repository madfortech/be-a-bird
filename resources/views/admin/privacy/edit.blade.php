        ...
        <section>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6 mt-4">
                <div class="p-4 sm:p-8 dark:bg-orange-800 shadow sm:rounded-lg">
                    <div class="max-w-xl">
                        <!-- create form -->
                        <form method="POST" action="{{ route("privacy.update", [$privacy->id]) }}" class="mt-6 space-y-6">
                            @method('PUT')    
                            @csrf
                            <div>
                                <x-input-label  for="title" :value="__('Title')"/>
                                <x-text-input id="title" name="title" type="text" 
                                class="mt-1 block w-full" value="{{ old('title', $privacy->title) }}"/>
                            ...
                            <x-primary-button>{{ __('Update Privacy') }}</x-primary-button>
